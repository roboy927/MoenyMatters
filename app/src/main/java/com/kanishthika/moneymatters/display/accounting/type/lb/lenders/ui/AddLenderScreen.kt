...
fun AddLenderScreen(
    ...
    val lenderUiState by lenderModel.addLenderUiState.collectAsState()
    ...
    val contactLauncher = rememberLauncherForActivityResult(
        ...
        contactUri?.let {
            lenderModel.updateLenderName(getContactName(context.contentResolver, contactUri))
            getContactPhoneNumber(context.contentResolver, contactUri)?.let { string ->
                lenderModel.updateLenderContactNumber(
                    string
                )
            }
        }
    ...
    Scaffold(
        ...
        topBar = { MMTopAppBar(titleText = "Add Lender") },
        bottomBar = {
            BottomAppBar(
                ...
                modifier = modifier
                    ...
                    .fillMaxWidth()
                    .clickable {
                        if (lenderModel.isAnyFieldIsEmpty(lenderUiState)){
                          Toast.makeText(context, "Please fill all fields", Toast.LENGTH_SHORT).show()
                        } else {
                            lenderModel.addLender()
                        }
                    }
            ) {
                Column(
        ...
        MMColumnScaffoldContentColumn(modifier = modifier, scaffoldPaddingValues = paddingValue) {
            ...
            MMOutlinedTextField(
                modifier = modifier.fillMaxWidth(),
                value = lenderUiState.lenderName,
                onValueChange = { input ->
                    lenderModel.updateLenderName(input)
                },
                labelText = "Lender Name",
                supportingText = { Text(text = "Enter a new Lender name or directly select from contacts below ") },
            ...
            MMOutlinedTextField(
                modifier = modifier.fillMaxWidth(),
                value = lenderUiState.lenderContactNumber,
                onValueChange = { input ->
                    lenderModel.updateLenderContactNumber(input)
                },
                labelText = "Lender Contact",
                keyboardOptions = KeyboardOptions(
            ...
            MMOutlinedTextField(
                ...
                onValueChange = { input ->
                    lenderModel.updateAmount(input)
                },
